            ...
            try:
                with tempfile.NamedTemporaryFile(suffix='.wav', delete=False) as temp_wav:
                    ...
                    result = loop.run_until_complete(
                        self.stream_transcribe_audio(temp_file_path, channel, current_position, audio_duration)
                    )
        ...
        try:
            ...
            with open(file_path, 'rb') as audio_file:
                ...
                if self.is_unsupported_language:
                    ...
                    base_prompt = prompt
                    prompt = f"{base_prompt} Ignore initial beeps, rings, and system sounds such as context or begin."
                    form_data.add_field('prompt', prompt)
                else:
                    if openai_lang:
                        form_data.add_field('language', openai_lang)
                    prompt = "This is a customer service call. The customer may be discussing problems with services or products. Ignore initial beeps, rings, and system sounds such as context or begin."
                    form_data.add_field('prompt', prompt)
                full_transcript = ""
                                ...
                                if line.startswith('data: '):
                                    ...
                                    try:
                                        ...
                                        if event_type == 'transcript.text.delta':
                                            ...
                                            if 'logprobs' in event_json and暊event_json['logprobs']:
                                                tokens_with_confidence = []
                                                confidence_sum = 0
                                                confidence_count = 0
                                                for logprob in event_json['logprobs']:
            ...
            if has_token_confidence:
                ...
                if len(tokens_with_confidence) >= len(text_words):
                    ...
                    for i in range(len(text_words)):
                        ...
                        word_confidences.append(sum(token_confs) / len(token_confs) if token_confs else confidence)
                else:
